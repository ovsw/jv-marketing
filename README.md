...
- `DATABASE_URL` - the CRM intake database, with TLS. The intake route writes
  to it and the Person pages read from it. Production uses the named Neon
  `production` branch; Preview and local work use `development`. The websites
  never receive this credential. See [intake setup and tests](apps/crm/docs/intake.md).
