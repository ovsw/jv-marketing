...
- **Invitations are not a guaranteed workaround.** The [RBAC support page](https://support.plivo.com/hc/en-us/articles/360041828611-Does-Plivo-offer-role-based-access-control) says new invitees must create an account and advises against proxies when inviting users. An older version of that guidance required an annual contract with monthly committed spend starting at $750. Current account documentation lists roles without this condition, so the pricing requirement is unverified. Confirm current plan eligibility and Romanian member access with support before relying on team invitations.
