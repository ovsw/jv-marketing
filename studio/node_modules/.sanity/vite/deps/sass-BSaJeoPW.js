import { keywords } from "./css-CJuWI0-T.js";
...
function tokenBase(stream, state) {
	...
	if (!state.cursorHalf) {
		...
		if (stream.eatWhile(/[\w-]/)) if (stream.match(/ *: *[\w-\+\$#!\("']/, false)) {
			...
			if (propertyKeywords.has(prop)) return "property";
			else if (propertyKeywords.has(word)) {
				state.prevProp = word;
				return "property";
			} else if (fontProperties.has(word)) return "property";
			return "tag";
		...
		if (stream.eatWhile(/[\w-]/)) {
			if (isEndLine(stream)) state.cursorHalf = 0;
			word = stream.current().toLowerCase();
			if (valueKeywords.has(word)) return "atom";
			else if (colorKeywords.has(word)) return "keyword";
			else if (propertyKeywords.has(word)) {
				state.prevProp = stream.current().toLowerCase();
				return "property";
			} else return "tag";
